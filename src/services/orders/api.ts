...
export async function fetchOrderFulfillStatusOptions(category: string) {
  const payload = await request<{ data?: unknown; status?: boolean }>(
    `${API_ENDPOINTS.ORDER_FULFILL_STATUSES}?${toQueryString({ category })}`
  )
...
export async function fetchStyleOptions(category: string) {
  return fetchVariantOptions(
    category === 'print' ? API_ENDPOINTS.PRINT_STYLES : API_ENDPOINTS.STYLES
  )
}
...
export async function fetchProductSizes(productId?: string, color?: string) {
  if (!productId || !color) return []
  const payload = await request<{ data?: unknown }>(
    `${API_ENDPOINTS.SIZES}?${toQueryString({ product_id: productId, color })}`
  )
  return mapListOptions(payload.data)
}
...
export async function fetchColorOptions(category: string, style?: string) {
  return fetchVariantOptions(
    category === 'print' ? API_ENDPOINTS.PRINT_COLORS : API_ENDPOINTS.COLORS,
    { style }
  )
}
...
export async function fetchSizeOptions(
  ...
  return fetchVariantOptions(
    category === 'print' ? API_ENDPOINTS.PRINT_SIZES : API_ENDPOINTS.SIZES,
    { style, color }
  )
}
