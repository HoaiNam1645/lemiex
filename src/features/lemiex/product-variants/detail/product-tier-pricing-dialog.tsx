...
const PRICE_TYPES = {
  production: [
    ...
    { key: 'back', label: 'Back' },
    { key: 'sleeve_left', label: 'Sleeve Left' },
    { key: 'sleeve_right', label: 'Sleeve Right' },
  ],
  shipping: [
...
export function ProductTierPricingDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-h-[90vh] max-w-[1200px] overflow-hidden rounded-[8px] p-0 shadow-xl'>
                ...
                <table className='w-full min-w-[920px] text-sm'>
                  <thead className='bg-muted/30'>
                    <tr className='border-b'>
                      <th className='px-4 py-3 text-left font-medium'>{m.type}</th>
                      {tiers.map((tier) => (
                      ...
                      <tr key={row.key} className='border-b last:border-b-0'>
                        ...
                        {tiers.map((tier) => (
                          <td key={tier.id} className='px-4 py-3'>
                            <Input
                              className='h-10 rounded-[6px] text-[14px]'
                              type='number'
                              step='0.01'
                              min={0}
                              readOnly={readOnly}
