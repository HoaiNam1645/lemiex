...
import { ArrowLeft, LoaderCircle, Ticket, Truck, Video } from 'lucide-react'
          ...
          <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
            ...
            <div className='flex flex-wrap items-center gap-2'>
              {role === 'Admin' || role === 'Staff' ? (
                ...
                <>
                  <Badge
                    className={`rounded-[6px] px-3 py-1.5 text-[13px] font-semibold ${getStatusTone(order.fulfill_status)}`}
                  >
        ...
        <div className='grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.85fr)]'>
          <div className='space-y-5'>
            <Card className={sectionCardClassName()}>
              ...
              <CardContent className='grid gap-4 px-4 py-4 md:grid-cols-2 xl:grid-cols-3'>
                <InfoItem
            ...
            <Card className={sectionCardClassName()}>
              ...
              <CardContent className='grid gap-4 px-4 py-4 md:grid-cols-2'>
                  ...
                  <InfoItem
                    ...
                    value={
                      <a
                        ...
                        target='_blank'
                        rel='noreferrer'
                        className='inline-flex rounded-[6px] bg-violet-50 px-2 py-1 text-[12px] font-semibold text-violet-700'
                      >
                ...
                {canSeeSeller && order.convert_label ? (
                  <InfoItem
                    label={detailMessages.convertLabel}
                    value={
                      <a
                        href={order.convert_label}
                        target='_blank'
                        rel='noreferrer'
                        className='inline-flex rounded-[6px] bg-fuchsia-50 px-2 py-1 text-[12px] font-semibold text-fuchsia-700'
                      >
            ...
            {canSeeSeller && qrCodes.length > 0 ? (
              <Card className={sectionCardClassName()}>
                ...
                <CardContent className='grid gap-3 px-4 py-4 sm:grid-cols-2 xl:grid-cols-3'>
                ...
                <CardContent className='grid gap-3 px-4 py-4 sm:grid-cols-2 xl:grid-cols-3'>
                  {mergeImages.map((image, index) => (
                    <a
                      ...
                      target='_blank'
                      rel='noreferrer'
                      className='space-y-2 rounded-[6px] border border-border/80 p-3'
                    >
                      <img
                        src={image.url}
                        alt={image.label}
              ...
              <Card className={sectionCardClassName()}>
                ...
                <CardContent className='space-y-3 px-4 py-4 text-[13px]'>
                  <div className='flex items-center justify-between'>
                    <span>{detailMessages.printCost}</span>
                    <span>{formatCurrency(order.pricing.print_cost)}</span>
                  </div>
                  <div className='flex items-center justify-between'>
                    <span>{detailMessages.shippingCost}</span>
                    <span>{formatCurrency(order.pricing.shipping_cost)}</span>
                  </div>
                  {(order.pricing.extra_fee || 0) > 0 ? (
                    <div className='flex items-center justify-between'>
                      <span>{detailMessages.extraFee}</span>
                      <span>{formatCurrency(order.pricing.extra_fee)}</span>
                    </div>
                  ) : null}
                  {(order.pricing.refund_fee || 0) > 0 ? (
                    <div className='flex items-center justify-between text-rose-600'>
                      <span>{detailMessages.refundFee}</span>
                      <span>-{formatCurrency(order.pricing.refund_fee)}</span>
                    </div>
                  ) : null}
                  <div className='flex items-center justify-between border-t border-border/70 pt-3 font-semibold'>
                    ...
                    <span>{formatCurrency(order.pricing.total_cost)}</span>
                  </div>
                </CardContent>
              </Card>
            ) : null}
            ...
            <Card className={sectionCardClassName()}>
              ...
              <CardContent className='flex flex-col gap-2 px-4 py-4'>
                <Button
                  type='button'
                  variant='outline'
                  className='w-full justify-start rounded-[6px] sm:w-auto'
