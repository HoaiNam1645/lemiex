        ...
        <div className='grid gap-4 xl:grid-cols-[minmax(0,1fr)_220px]'>
          <div className='space-y-4'>
            ...
            <div className='grid gap-4 lg:grid-cols-2'>
              ...
              <div className='space-y-1.5'>
                <label className='text-[12px] font-medium text-foreground'>
                  {createFormMessages.mockupSleeveLeft}
                </label>
                <LemiexFileUploadInput
                  value={item?.mockup_sleeve_left || ''}
                  onChange={(value) =>
                    setValue(`line_items.${index}.mockup_sleeve_left`, value, {
                      shouldDirty: true,
                      shouldTouch: true,
                    })
                  }
                  type='mockup'
                  placeholder={createFormMessages.placeholders.sleeveLeft}
                />
              </div>
              <div className='space-y-1.5'>
                <label className='text-[12px] font-medium text-foreground'>
                  {createFormMessages.mockupSleeveRight}
                </label>
                <LemiexFileUploadInput
                  value={item?.mockup_sleeve_right || ''}
                  onChange={(value) =>
                    setValue(`line_items.${index}.mockup_sleeve_right`, value, {
                      shouldDirty: true,
                      shouldTouch: true,
                    })
                  }
                  type='mockup'
                  placeholder={createFormMessages.placeholders.sleeveRight}
                />
              </div>
            </div>
          </div>
            ...
            {printFileFields.map((field, fileIndex) => (
              ...
              >
                ...
                <div className='grid gap-4 sm:grid-cols-2 xl:grid-cols-4'>
                  ...
                  <div className='space-y-1.5'>
                    <label className='text-[12px] font-medium text-foreground'>
                      {createFormMessages.embroideryType}
                    </label>
                    <Select
                      value={
                        watch(
                          `line_items.${index}.print_files.${fileIndex}.embroidery_type`
                        ) || 'standard'
                      }
                      onValueChange={(value) =>
                        setValue(
                    ...
                    >
                      <SelectTrigger className='w-full rounded-[6px] text-[13px]'>
                        <SelectValue
                          placeholder={
                            createFormMessages.placeholders.selectEmbroideryType
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className='space-y-1.5'>
                    <label className='text-[12px] font-medium text-foreground'>
                      {createFormMessages.embFileUrl}
                    </label>
                    <LemiexFileUploadInput
                      value={
                        watch(
                          `line_items.${index}.print_files.${fileIndex}.url_emb`
                        ) || ''
                      }
                      onChange={(value) =>
                        setValue(
                      ...
                      type='design'
                      accept='.emb'
                      placeholder={createFormMessages.placeholders.embFile}
                    />
                  </div>

                  <div className='space-y-1.5'>
                    <label className='text-[12px] font-medium text-foreground'>
                      {createFormMessages.pesFileUrl}
                    </label>
                    <LemiexFileUploadInput
                      value={
                        watch(
                          `line_items.${index}.print_files.${fileIndex}.url_pes`
                        ) || ''
                      }
                      onChange={(value) =>
                        setValue(
                      ...
                      type='design'
                      accept='.pes'
                      placeholder={createFormMessages.placeholders.pesFile}
                    />
                  </div>
                </div>
                ...
                <input
                  type='hidden'
                  {...register(`line_items.${index}.print_files.${fileIndex}.url`)}
                />
              </div>
            ))}
          </div>
...
export function ShirtOrderForm({ mode }: ShirtOrderFormProps) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoadingMeta(true)
      try {
        const [storesData, prioritiesData, embroideryData, shippingMethodsData] =
          await Promise.all([
            fetchStores(),
            fetchFulfillmentPriorities(),
            fetchEmbroideryTypesMetadata(),
        ...
        setEmbroideryTypes([
          {
            value: 'standard',
            label: optionLabels.embroideryType.standard,
          },
        ])
        setShippingMethods([
          {
  ...
  }, [form, mode, optionLabels.embroideryType.standard, setValue])
    ...
    try {
      ...
      const lineItems = values.line_items.map((item) => ({
        ...
        print_files: item.print_files.map((file) => ({
          ...file,
          url: file.url || '',
          url_emb: file.url_emb || '',
          url_pes: file.url_pes || '',
          embroidery_type: file.embroidery_type || 'standard',
        })),
      }))
              ...
              <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-3'>
                ...
                <div className='space-y-1.5 xl:col-span-3'>
                  ...
                  <Textarea
                    ...
                    className='min-h-[88px] rounded-[6px] text-[13px]'
                  />
                </div>
              </div>
            </CardContent>
